...
/// serialize a `Frame` into a `Vec<u8>`
pub fn frame_serializer(frame: &Frame) -> Vec<u8> {
...
#[cfg(test)]
mod tests {
    use crate::{serializer::frame_serializer, Frame};

    #[test]
    fn test_serialize() {
        let testcase = Frame::new(66, 0, 7, 87890416, vec![0, 0, 15]);
        let want = vec![220, 128, 66, 14, 7, 5, 61, 25, 240, 0, 0, 15, 29, 116];
        assert_eq!(want, frame_serializer(&testcase));
    }

    #[test]
    fn test_create_get_request() {
        let testcase = Frame::new_get(0, 66, 87890416);
        let want = vec![220, 194, 0, 11, 6, 61, 5, 25, 240, 36, 62];
        assert_eq!(want, frame_serializer(&testcase));
    }

    #[test]
    fn test_create_set_request() {
        let testcase = Frame::new_set(0, 66, 87884342, vec![1, 0]);
